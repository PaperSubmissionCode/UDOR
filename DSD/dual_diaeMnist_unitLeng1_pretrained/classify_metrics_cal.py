...
unitlLength=1
data_npz_path='../../npz_DSD_dataset/codes/codes_DSD_codesAndImgForMetricsCal.npz'
label_npz_path='../../npz_DSD_dataset/Arr1_mnistImgsForDSD_GTimgs_mask_GTlabel_(47x64)x32x32x1_unitLength1_dataset.npz'
...
model_0.fit(data_train[:,unitlLength:2*unitlLength], train_label_0)
pre_0 = model_0.predict_proba(data_test[:,unitlLength:2*unitlLength])
pre1_0 = model_0.predict(data_test[:,unitlLength:2*unitlLength])


## for 1
model_1 = SVC(kernel='linear', probability=True)
train_label_1=np.where(label_train[:,1]==-1,0,1)
model_1.fit(data_train[:,0:unitlLength], train_label_1)
pre_1 = model_1.predict_proba(data_test[:,0:unitlLength])
pre1_1 = model_1.predict(data_test[:,0:unitlLength])

# for 2
model_2 = SVC(kernel='linear', probability=True)
train_label_2=np.where(label_train[:,2]==-1,0,1)
model_2.fit(data_train[:,2*unitlLength:3*unitlLength], train_label_2)
pre_2 = model_2.predict_proba(data_test[:,2*unitlLength:3*unitlLength])
pre1_2 = model_2.predict(data_test[:,2*unitlLength:3*unitlLength])
