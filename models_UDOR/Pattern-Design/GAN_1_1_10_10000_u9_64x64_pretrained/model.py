...
from lib.models.nets_64x64_small import NetsRetreiverWithClassifier
...
class AE(object):
    ...
    def __build_graph(self):
        ...
        # vector related
        # self.base_vec1=tf.placeholder(tf.float32, shape=[None] + list([self.latent_dim]))
        # self.base_vec2=tf.placeholder(tf.float32, shape=[None] + list([self.latent_dim]))
        # self.base_vec3=tf.placeholder(tf.float32, shape=[None] + list([self.latent_dim]))
        self.base_mk1=tf.placeholder(tf.float32, shape=[None] + list([self.latent_dim]))
        self.base_mk2=tf.placeholder(tf.float32, shape=[None] + list([self.latent_dim]))
        self.base_mk3=tf.placeholder(tf.float32, shape=[None] + list([self.latent_dim]))
        self.vec_gt=tf.placeholder(tf.float32, shape=[None] + list([1]))
        # classification gt
        ...
        # base vector
        # self.r1_part1=tf.nn.l2_normalize(tf.add(tf.multiply(self.z1,self.base_mk1),self.base_vec1),axis=1,epsilon=1e-12)
        # self.r1_part2=tf.nn.l2_normalize(tf.add(tf.multiply(self.z1,self.base_mk2),self.base_vec2),axis=1,epsilon=1e-12)
        # self.r1_part3=tf.nn.l2_normalize(tf.add(tf.multiply(self.z1,self.base_mk3),self.base_vec3),axis=1,epsilon=1e-12)

        # self.part1_sum=tf.multiply(self.r1_part1,self.base_vec1)
        # self.part2_sum=tf.multiply(self.r1_part2,self.base_vec2)
        # self.part3_sum=tf.multiply(self.r1_part3,self.base_vec3)

        # set representation all 000
        self.r_zero=tf.multiply(self.z1,self.mask_zero)
        self.img_out_zero=self.__Dec(self.r_zero)
        # split latent representation and  classification
        r_part1,r_part2,r_part3=tf.split(self.r1,3,axis=1)
        ...
        # Loss and optimizer
        self.__prep_loss_optimizer(norm_x1,norm_img_black,r11,c_p0,c_p1,c_p2,c_p3,disc_real,disc_fake)   
    ...
    def __prep_loss_optimizer(self, norm_x1,norm_img_black,r11,c_p0,c_p1,c_p2,c_p3,disc_real,disc_fake):
        ...
        #[Loss3]vector base loss
        # vec_part1_loss =  tf.reduce_sum(tf.square(tf.reduce_sum(self.part1_sum, axis=1)-self.vec_gt), axis=1) 
        # vec_part2_loss =  tf.reduce_sum(tf.square(tf.reduce_sum(self.part2_sum, axis=1)-self.vec_gt), axis=1) 
        # vec_part3_loss =  tf.reduce_sum(tf.square(tf.reduce_sum(self.part3_sum, axis=1)-self.vec_gt), axis=1) 
        #[Loss4]with representation all 0  image loss
        reconstr_img_zero_loss =  tf.reduce_sum(tf.square(norm_img_black -self.img_out_zero), axis=1) 
        
        #[loss5] classification loss
        ...
        temp_2=self.vec_gt-tf.reduce_sum((self.class_gt1-self.class_gt1*c_p2),1)*tf.reduce_sum((self.class_gt3-self.class_gt3*c_p2),1)
        self.class2_loss=-tf.reduce_mean(tf.log(temp_2))
        temp_3=self.vec_gt-tf.reduce_sum((self.class_gt1-self.class_gt1*c_p3),1)*tf.reduce_sum((self.class_gt4-self.class_gt4*c_p3),1)
        self.class3_loss=-tf.reduce_mean(tf.log(temp_3))
        #zero input class 1
        self.class0_loss=-tf.reduce_mean(self.class_gt1*tf.log(c_p0))*3
        

        # average over batch
        self.rec_loss=1.0*tf.reduce_mean(reconstr_img_loss)
        self.reset0_loss =1.0* tf.reduce_mean(reconstr__rep_loss)
        # self.vector_loss=0* tf.reduce_mean(vec_part1_loss+vec_part2_loss+vec_part3_loss)
        self.rec_zero_loss=10*tf.reduce_mean(reconstr_img_zero_loss)
        self.class_loss=10000*(self.class1_loss+self.class2_loss+self.class3_loss+self.class0_loss)

        
        #self.loss=self.rec_loss+self.class_loss+self.vector_loss+self.rec_zero_loss+self.reset0_loss
        #self.loss=self.rec_loss
        #self.loss=self.rec_zero_loss
        #self.loss=self.rec_loss+self.reset0_loss+self.rec_zero_loss
        #self.loss=self.rec_loss+self.reset0_loss+self.rec_zero_loss+self.class_loss

        #self.loss=self.rec_loss+self.rec_zero_loss
        #self.loss=self.rec_loss+self.class_loss+self.rec_zero_loss
        #self.loss=self.rec_loss+self.reset0_loss+self.rec_zero_loss+self.class_loss
        #self.loss=self.rec_loss+self.reset0_loss+self.vector_loss+self.rec_zero_loss+self.class_loss
        self.loss=self.rec_loss+self.reset0_loss+self.rec_zero_loss+self.class_loss
        lr=self.lr
    ...
    def train(self, n_iters, n_iters_per_epoch, stats_iters, ckpt_interval):
        ...
        print(fixed_mk1[0:4])
        # replace mask
        unitLength=9  #(need to changed when has larger unitLength)
        #fixed_mk1=self.generateMask(self.batch_size,unitLength)

        # get vector related mask and base vector
        base_mask1,base_mask2,base_mask3,vector_gt=self.generateVectorBaseMaskGT(self.batch_size,unitLength)
        class_gt1,class_gt2,class_gt3,class_gt4=self.generateClassificationLabel(self.batch_size)
        # generate zero representation and white image
        img_zero,fixed_zero_mk=self.generateMaskZero(self.batch_size,unitLength)
        '''
        print("fixed_zero_mk=")
        print(fixed_zero_mk[0:4])
        print("img_zero=")
        print(img_zero[0])
        '''
        #
        ...
        #
        fixed_img_zero = self.session.run(tf.constant(img_zero))
        save_images(fixed_img_zero, os.path.join(self.dirs['samples'], 'samples_white_groundtruth.png'))
    ...
    def generateMask(self,batch_size,unitLength):
        #==============get mask==============
        maskArray=np.empty((batch_size,unitLength*3))
    ...
    def generateVectorBaseMaskGT(self,batch_size,unitLength):
        #==============get mask==============
        base_mask1=np.empty((batch_size,unitLength*3))
        base_mask2=np.empty((batch_size,unitLength*3))
        base_mask3=np.empty((batch_size,unitLength*3))
        mask1=np.zeros((unitLength*3))
        mask2=np.zeros((unitLength*3))
        mask3=np.zeros((unitLength*3))
        for k in range(unitLength,2*unitLength):
            mask1[k-unitLength]=1.0
            mask2[k]=1.0
            mask3[k+unitLength]=1.0
        # base_vec1=np.empty((batch_size,unitLength*3))
        # base_vec2=np.empty((batch_size,unitLength*3))
        # base_vec3=np.empty((batch_size,unitLength*3))
        # vec1=[0.5774,0.5774,0.5774,0,0,0,0,0,0]
        # vec2=[0,0,0,0.5774,-0.5774,-0.5774,0,0,0]
        # vec3=[0,0,0,0,0,0,-0.5774,0.5774,-0.5774]
        vector_gt=np.ones((batch_size,1))
        ...
        return base_mask1,base_mask2,base_mask3,vector_gt
    ...
    def generateMaskZero(self,batch_size,unitLength):
        ...
        maskArray=np.empty((batch_size,unitLength*3))
        mask=np.zeros((unitLength*3))
        for k in range(0,3*unitLength):
            mask[k]=0.0
        # reset value 0~64
        ...
        h=64
        ch=3
        #imgArray= np.ones((batch_size,ch,w,h))
        imgArray= np.ones((batch_size,ch,w,h))
    ...
    def generateClassificationLabel(self,batch_size):
        ...
        return class_gt1,class_gt2,class_gt3,class_gt4
