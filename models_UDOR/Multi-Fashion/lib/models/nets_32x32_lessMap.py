...
def resnet_encoder(name, inputs, n_channels, latent_dim, is_training, mode=None, nonlinearity=tf.nn.relu):
    ...
    output = tf.reshape(inputs, [-1, n_channels, DIM, DIM])
    #print output.shape
    output = Conv2D(name + '.Input', n_channels, 4*DIM, 3, output, he_init=False, cpu=CPU)
    #print output.shape
    output = ResidualBlock(name + '.Res1', 4*DIM, 4*DIM, 3, output, is_training, mode, resample='down')
    #print output.shape
    output = ResidualBlock(name + '.Res2', 4*DIM, 2*DIM, 3, output, is_training, mode, resample='down')
    #print output.shape
    output = ResidualBlock(name + '.Res3', 2*DIM, DIM, 3, output, is_training, mode, resample='down')
    #print output.shape
    #output = ResidualBlock(name + '.Res4', 8*DIM, 8*DIM, 3, output, is_training, mode, resample='down')    
    output = tf.reshape(output, [-1, 4*4*1*DIM])
    #print output.shape
    output = Linear(name + '.Output', 4*4*1*DIM, latent_dim, output)
    #print output.shape
    return output
