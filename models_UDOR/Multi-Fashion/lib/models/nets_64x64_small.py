...
def resnet_encoder(name, inputs, n_channels, latent_dim, is_training, mode=None, nonlinearity=tf.nn.relu):
    ...
    output = tf.reshape(output, [-1, 4*4*4*8*DIM])
    #print output.shape
    output = Linear(name + '.Output', 4*4*4*8*DIM, latent_dim, output)
    #print output.shape
    return output
...
def resnet_decoder(name, z, n_channels, is_training, mode=None, nonlinearity=tf.nn.relu):
    ...
    output = Linear(name + '.Input', z.get_shape().as_list()[1], 4*4*4*8*DIM, z)
    #print output.shape
    output = Normalize(name + '.BN0', [0], output, is_training, mode)
    #print output.shape
    output = nonlinearity(output)
    ...
    output = ResidualBlock(name + '.Res1', 8*DIM, 4*DIM, 3, output, is_training, mode, resample='up', norm_inputs=True)
    #print output.shape
    output = ResidualBlock(name + '.Res2', 4*DIM, 2*DIM, 3, output, is_training, mode, resample='up')
    #print output.shape
    output = ResidualBlock(name + '.Res3', 2*DIM, 1*DIM, 3, output, is_training, mode, resample='up')
    #print output.shape
