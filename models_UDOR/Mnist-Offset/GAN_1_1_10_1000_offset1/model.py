...
class AE(object):
    ...
    def __build_graph(self):
        ...
        self.img_white=tf.placeholder(tf.float32, shape=[None] + list(self.image_shape))
        # classification gt
        ...
        # Loss and optimizer
        self.__prep_loss_optimizer(norm_x1,norm_img_white,r11,c_p0,c_p1,c_p2,disc_real,disc_fake)   
    ...
    def __prep_loss_optimizer(self, norm_x1,norm_img_white,r11,c_p0,c_p1,c_p2,disc_real,disc_fake):
 
        norm_x1= tf.reshape(norm_x1, [-1, self.output_dim])
        norm_img_white= tf.reshape(norm_img_white, [-1, self.output_dim])
        ...
        reconstr_img_zero_loss =  tf.reduce_sum(tf.square(norm_img_white -self.img_out_zero), axis=1) 
        ...
        #zero input class 1
        self.class0_loss=-tf.reduce_mean(self.class_gt0*tf.log(c_p0))*2
        ...
        self.class_loss=1000*(self.class1_loss+self.class2_loss+self.class0_loss)
    ...
    def train(self, n_iters, n_iters_per_epoch, stats_iters, ckpt_interval):
        ...
        class_gt0,class_gt1,class_gt2,vector_one=self.generateClassificationLabelandVecOne(self.batch_size)
        ...
        for iteration in range(start_iter, n_iters):
            ...
            _, cost = self.session.run((self.optimizer, self.loss),feed_dict={self.x1: _data1,self.mask: _mask1,self.vec_one:vector_one,self.img_white:img_zero,self.mask_zero:fixed_zero_mk,self.class_gt0:class_gt0,self.class_gt1:class_gt1,self.class_gt2:class_gt2,self.is_training:True})
            running_cost += cost
            #==generator training
            _,_disc_gene_cost=self.session.run((self.gen_train_optimizer,self.gen_cost),feed_dict={self.x1: _data1,self.x_gan:_gan_data,self.mask: _mask1,self.vec_one:vector_one,self.img_white:img_zero,self.mask_zero:fixed_zero_mk,self.class_gt0:class_gt0,self.class_gt1:class_gt1,self.class_gt2:class_gt2,self.is_training:True})
    ...
    def visualise_reconstruction_path(self,path, X1, mk1, iteration):
        X_r1, X_r0 = self.reconstruct(X1, mk1)
        # print(X_r0[3])
        X_r1 = ((X_r1 + 1.) * (255.99 / 2)).astype('int32').reshape([-1] + self.image_shape)
        X_r0 = ((X_r0 + 1.) * (255.99 / 2)).astype('int32').reshape([-1] + self.image_shape)
        save_images(X_r1, os.path.join(path, str(iteration) + 'samples_reconstructed.png'))
        save_images(X_r0, os.path.join(path, str(iteration) + 'reset0_reconstructed.png'))
    ...
    def generateMaskZero(self,batch_size,unitLength):
        #==============get mask==============
        maskArray=np.empty((batch_size,unitLength*2))
        mask=np.zeros((unitLength*2))
        # reset value 0~64
    ...
    def generateClassificationLabelandVecOne(self,batch_size):
        ...
        return class_gt0,class_gt1,class_gt2,vector_one
